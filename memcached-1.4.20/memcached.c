...
static rel_time_t realtime(const time_t exptime) {
    ...
    if (exptime > REALTIME_MAXDELTA) {
		...
		//大于REALTIME_MAXDELTA表明过期时间是绝对时间
...
static int add_msghdr(conn *c)
{
	...
	//iovused--使用的个数，c->iovused第一个未使用的元素
...
static int ensure_iov_space(conn *c) {
    ...
    if (c->iovused >= c->iovsize) {
        ...
        for (i = 0, iovnum = 0; i < c->msgused; i++) {
			//因为msg_iov存储顺序和c->iov一致
			//所以iovnum的移动大小为msg_iovlen
            c->msglist[i].msg_iov = &c->iov[iovnum];
			//iovnum移动的距离为msg_iovlen
			//msghdr里面有多个iovec
			//有些高级api针对iovec，有些高级api针对msghdr
...
static int add_iov(conn *c, const void *buf, int len) {
    ...
    do {
         ...
         */
		//1==c->msgused什么含义？当前m为数组第0个元素
		//第一个msg限制总大小，其它msg限制iovlen
        limit_to_mtu = IS_UDP(c->transport) || (1 == c->msgused);

        /* We may need to start a new msghdr if this one is full. */
		//msgbytes--当前msg中数据的大小
        if (m->msg_iovlen == IOV_MAX ||
            (limit_to_mtu && c->msgbytes >= UDP_MAX_PAYLOAD_SIZE)) {
            //add_msghdr里面有c->msgused++，切换到一个新的msg
			//udp时，里面也递归调用了add_iov()
			add_msghdr(c);
            m = &c->msglist[c->msgused - 1];
        }
        ...
        //c->msgbytes--number of bytes in current msg
		//msgbytes当前msg已有的数据大小
...
enum store_item_type do_store_item(item *it, int comm, conn *c, const uint32_t hv) {
    ...
    if (old_it != NULL && comm == NREAD_ADD) {
		...
		//promote--提升
		//do_item_update--只做了更新链表操作，没操作hash桶
        ...
        || comm == NREAD_APPEND || comm == NREAD_PREPEND))
    {
        ...
        if (comm == NREAD_APPEND || comm == NREAD_PREPEND) {
            ...
            if (stored == NOT_STORED) {
				...
				//it->nbytes + old_it->nbytes - 2--新的大小
...
enum delta_result_type do_add_delta(conn *c, const char *key, const size_t nkey,
     ...
     * increase. */
	//it->refcount==2代表什么？表示目前只有当前get和hash桶引用？
    if (res + 2 <= it->nbytes && it->refcount == 2) { /* replace in-place */
		...
		//buf后面一个字节开始，拷贝空格，直到nbytes-2前面一个地址即nbytes-2地址
...
static enum try_read_result try_read_network(conn *c) {
    ...
    if (c->rcurr != c->rbuf) {
        if (c->rbytes != 0) /* otherwise there's nothing to copy */
			//rcurr--未解析的数据
			//rbytes--从rcurr开始有多少数据
            memmove(c->rbuf, c->rcurr, c->rbytes);
        c->rcurr = c->rbuf;
    }

    while (1) {
		//rbuf的大小，开始不会比rsize大，后面经过处理会变大，但buf不会溢出
        if (c->rbytes >= c->rsize) {
            if (num_allocs == 4) {
                return gotdata;
            }
...
static enum transmit_result transmit(conn *c) {
    ...
    if (c->msgcurr < c->msgused) {
        ...
        if (res > 0) {
			...
			//res发送的字节数，msg_iov->iov_len相当于msg_iov[0].iov_len
