...
typedef struct {
	...
	//slab_list数组存储每个新分配的内存片的地址
    void **slab_list;       /* array of slab pointers */
	
	//list_size是slab_list数组的总大小
...
unsigned int slabs_clsid(const size_t size) {
	...
	//找到第一个size<=的res
	//当res超过power_largest时，返回0，表示没找到
...
void slabs_init(const size_t limit, const double factor, const bool prealloc) {
    ...
    while (++i < POWER_LARGEST && size <= settings.item_size_max / factor) {
		...
		//去整，因此perslab*size不一定等于settings.item_size_max
	...
	//当size <= settings.item_size_max / factor条件false时
	//power_largest！=i，修改power_largest
    power_largest = i;
    ...
    if (prealloc) {
		//power_largest--slabclass数组的最大值
		//参数可以控制给哪些slabclass预分配
...
//把一夜地址分成perslab个item，放入p->slots
static void split_slab_page_into_freelist(char *ptr, const unsigned int id) {
    slabclass_t *p = &slabclass[id];
    int x;
	//perslab--how many items per slab
    for (x = 0; x < p->perslab; x++) {
		//ptr每次移动的距离为1个item,将item放入p->slots
...
static int do_slabs_newslab(const unsigned int id) {
    slabclass_t *p = &slabclass[id];
	//reassign--再分配，和slab_reassign参数有关
	//表示内存可以分配给别的slabclass变量
	//先做乘法再做3目运算符
	//len为一个内存片大小
...
static void *do_slabs_alloc(const size_t size, unsigned int id) {
	...
	//slabs_clsid == 0表示没有在hash桶和lru链表中
    assert(p->sl_curr == 0 || ((item *)p->slots)->slabs_clsid == 0);

    /* fail unless we have space at the end of a recently allocated page,
       we have something on our freelist, or we could allocate a new page */
    //p->sl_curr等于0时，表示没有空闲item，需要新分配一个内存片
...
//ptr为一个item的地址，这个item会放入p->slots
static void do_slabs_free(void *ptr, const size_t size, unsigned int id) {
    slabclass_t *p;
    item *it;
...
static void do_slabs_stats(ADD_STAT add_stats, void *c) {
    ...
    for(i = POWER_SMALLEST; i <= power_largest; i++) {
        ...
        if (p->slabs != 0) {
			...
			//item都存在链表里，所以总的item数这么求slabs*perslab
...
//设置slab_rebal.slab_start和slab_rebal.slab_end
static int slab_rebalance_start(void) {
	...
	//do_item_get时，如果item在rebalance的内存页中，则unlink&free
...
static int slab_rebalance_move(void) {
	...
	//循环slab_bulk_check次，相当于找slab_bulk_check个item
    for (x = 0; x < slab_bulk_check; x++) {
		//当前rebalance的内存页
        item *it = slab_rebal.slab_pos;
        status = MOVE_PASS;
		//it->slabs_clsid==255表示已经move_done了，在内存页中，但是不在slots链表中了
...
static void slab_rebalance_finish(void) {
	...
	//把最后一页内存地址拷贝到s_cls->killing-1位置，killing只设置为1，没看到其它设置
	//slab_list[s_cls->slabs-1]value没变，但slabs--了
